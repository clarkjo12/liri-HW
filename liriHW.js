        ...
        reqInquirer
            .prompt([
                ...
                var spotify = new Spotify({
                  ...
                  spotify.search({ type: 'track', query: inquirerResponse.songinput, limit: "3" }, function(err, data) {
                    if (err) {
...
fs.readFile("random.txt","utf8", function(err, data) {
    ...
    for (var i = 0; i < output.length; i++) {
     ...
     var spotify = new Spotify({
      ...
      spotify.search({ type: 'track', query: secondArg, limit: "3" }, function(err, data) {
        if (err) {
          return console.log('Error occurred: ' + err);
        }
                    console.log("\n Artist(s)/Band: "+ data.tracks.items[0].album.artists[0].name);
